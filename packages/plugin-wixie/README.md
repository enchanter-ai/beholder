...
**Placeholder.** Wixie is the prompt-engineering meta-engine that runs the research → craft → converge → harden → translate lifecycle behind Beholder's architecture spec. It is currently a separate companion repo at <https://github.com/enchanter-ai/wixie> and does **not** yet ship a runtime `PluginAdapter` against the Beholder event bus.
