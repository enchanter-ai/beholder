...
import { buildCorpora } from './hydra/veto-core.mjs';
...
function guardActionAtTrustGate(event: EnchantedEvent): PluginAck {
  // Corpus construction is shared with the auto-hook path via
  // hydra/veto-core.mjs::buildCorpora — matching multiple corpus views
  // defeats array-arg evasion:
  ...
  // Matching itself stays STATE-aware here (via matchCvePatterns) so a
  // configureHydra() override still takes effect on the SDK path.
  const payload = (event.payload ?? {}) as Record<string, unknown>;
  const tool = typeof payload['tool'] === 'string' ? (payload['tool'] as string) : '';
  const corpora = buildCorpora({ tool, args: payload['args'], raw: payload });
